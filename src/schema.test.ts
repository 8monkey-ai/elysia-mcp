...
describe("flattenSchemas", () => {
	it("merges params, query, and body into a flat schema", () => {
		const result = flattenSchemas("test_tool", {
			...
			query: Type.Object({ fields: Type.Optional(Type.String({ description: "Fields to include" })) }),
	...
	it("warns on name collisions", () => {
		...
		expect(result.warnings.some((w) => w.includes("collision") || w.includes("exists in both"))).toBe(true);
	});
